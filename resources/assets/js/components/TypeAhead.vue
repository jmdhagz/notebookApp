<template>
    <div :class="typeaheadState">
        ...
        <ul class="typeahead__list" ref="list" v-if="open">
            <li class="typeahead__item" v-for="(collaborator, index) in collaboratorLists" :key="index">
                <a class="typeahead__link" @mousedown.prevent="select(collaborator)">
        ...
        computed: {
            ...
            collaboratorLists() {
                ...
                return this.lists.filter((list) => {
                    return ( exp.test(list.id) || exp.test(list.first_name))
                })
            }
        },
        methods: {
            onDownKey() {
                if(this.collaboratorLists.length -1 > this.selectIndex) {
                    this.selectIndex++
            ...
            onEnterKey() {
                const option = this.collaboratorLists[this.selectIndex]
                if(option) {
                    this.select(option)
                }
            },
            select(collaborator) {
                this.displayText = collaborator.first_name,
                this.$emit('input', (collaborator.id))
                this.$refs.search.blur()
                this.$emit('selectedColaborator', (collaborator.id))
            },
            toggle(e) {
